...
class Pokedex:
    ...
    def getPokemonsByWeaknesses(self, type: list):
        response = self.collection.find({"weaknesses": {"$all": type}})
        result = []
        for pokemon in response:
            result.append(pokemon)
        return result
    ...
    def getPokemonsByType(self, type: list):
        response = self.collection.find({ "type": {"$in": type}, "next_evolution": {"$exists": True} })
        result = []
        for pokemon in response:
            result.append(pokemon)
        return result

    def getPokemonsByFire(self):
        response = self.collection.find({"$or": [{"type": "Fire"}, {"weaknesses": "Fire"}]})
        result = []
        for pokemon in response:
            result.append(pokemon)
        return result

    def getPokemonsByWeaknessesOne(self):
        response = self.collection.find({"weaknesses": {"$size": 1}})
        result = []
        for pokemon in response:
            result.append(pokemon)
        return result
